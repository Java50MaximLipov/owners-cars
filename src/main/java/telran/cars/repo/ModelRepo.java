...
public interface ModelRepo extends JpaRepository<Model, ModelYear> {
	@Query(value = "select model_name from cars join trade_deals td"
			...
			+ "(select count(*) as count from cars join trade_deals " + " on cars.car_number = trade_deals.car_number "
	...
	@Query(value = "select model_name as name, count(*) as amount "
			+ "from (select * from car_owners where birth_date between :birthDate1 and :birthDate2)" + " owners_age"
			+ " join cars where id=owner_id "
			+ "group by model_name order by count(*) desc limit :nModels", nativeQuery = true)
	List<ModelNameAmount> findPopularModelNameOwnerAges(int nModels, LocalDate birthDate1, LocalDate birthDate2);
